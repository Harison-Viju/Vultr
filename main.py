...
st.markdown("""
    ...
    .text-box:focus-within {
        border-color: #3399ff;
        box-shadow: 0px 0px 8px rgba(51, 153, 255, 0.5);
    }
    .text-box:hover {
        transform: translateY(-3px);
        box-shadow: 0px 6px 15px rgba(0, 123, 255, 0.6);
...
# Display main header
st.markdown('<div class="main-header">Kerala Disaster Alert System</div>', unsafe_allow_html=True)
...
for district, info in predictions.items():
    if info["risk"] == "High Risk":
        st.sidebar.write(f"<div class='high-risk'>{district} - {info['probability']:.2%}</div>", unsafe_allow_html=True)



# Chatbot section

st.sidebar.markdown('<div class="text-box">Chatbot - Disaster Alert System</div>', unsafe_allow_html=True)
